...
- `reference`: **the baseline / contrast arm.** Unlike `intervention` (I-only), the reference may be
  ...
  pattern compares one family/condition against another (e.g. "ViT vs U-Net"), select the C.. node
  naming the contrasted family/condition when no distinct I.. baseline arm exists — **preserve the
  contrast arm even when the values are bundled in the pattern note.** (e.g. pattern "ViT models show
  higher accuracy than U-Net" → reference = the U-Net condition node.) Never use a P.. or E.. node as
