...
TRANSCRIBE_PROMPT = (
    "You are transcribing one page range of a scientific paper. Output has two parts.\n\n"
    "STEP 1 — Figures & tables. Output a SINGLE fenced ```json block of exactly this shape, "
    "enumerating every FIGURE and TABLE printed on these pages:\n"
    "```json\n"
    "{\"figures\": [{\"id\": \"Figure 1\", \"caption\": \"...\", \"description\": \"...\"}], "
    "\"tables\": [{\"id\": \"Table 1\", \"caption\": \"...\", \"columns\": [\"...\"], "
    "\"rows\": [{\"col\": \"cell\"}], \"reads\": \"...\"}]}\n"
    "```\n\n"
    "RULES FOR TABLES — transcribe, do NOT summarize:\n"
    "- `caption`: the printed caption verbatim.\n"
    "- `columns`: the exact column headers in order, INCLUDING the leading row-label column. Put "
    "the unit into the header when the table shows one (e.g. \"acc (%)\", \"#params (M)\").\n"
    "- `rows`: ONE object per data row, keyed by the column headers, every cell VERBATIM (exact "
    "numbers, keep decimals, keep the row-label cell). Do not round, do not collapse a range, do "
    "not drop rows. A cell marked best/bold gets a \" (best)\" suffix; an empty cell is \"\".\n"
    "- `reads`: ONE short sentence naming the comparison and which way the numbers move. This is "
    "an EXTRA, never a replacement for `rows`.\n\n"
    "RULES FOR FIGURES — describe densely and quantitatively:\n"
    "- `caption`: the printed caption verbatim.\n"
    "- `description`: read the actual content, not just the caption. For a plot: name the x and y "
    "axes WITH units, list each series/curve with its start and end values, any crossing or peak, "
    "whether it rises/falls/flattens, and EVERY number printed on the figure. For an example/grid "
    "figure: what each panel shows. End with the one-line qualitative trend.\n\n"
    "Use each figure/table's printed id verbatim. If there are none on these pages, use empty "
    "arrays. Do NOT invent rows, numbers, or figures that are not on these pages.\n\n"
    "STEP 2 — Prose. After the closing fence, transcribe ALL running text of these pages verbatim "
    "in reading order (title, abstract, body, captions, footnotes, appendix). Do not summarize, "
    "translate, or add commentary. Do not wrap the prose in code fences."
)
