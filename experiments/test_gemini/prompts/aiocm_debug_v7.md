...
one of six categories. These six categories realize the five AIO node types A (assumption),
M (mechanism), I (intervention), C (context), and O (observable). The observable O is never a
single span: it is captured through its two measurable halves — eval_metric (E) and pattern
(P) — which a later Layer-2 stage composes into O[E, P]. So you still emit E and P as separate
spans below. Use exactly these definitions — do not redefine them in your own words:
...
- "eval_metric": The concrete measurement method, unit, or evaluation protocol used to quantify
  ...
  "context".
- "pattern": The DIRECTION or SHAPE the measurements took across conditions — captured as a
  TREND, not as a raw number. The "text" must be a trend only: prefer one of "up" / "down" /
  "flat" / "mixed", or a short shape phrase such as "monotonic increase", "degradation from A
  to B", "no change". Do NOT put numeric magnitudes in the pattern text (no "+3.2 points", no
  "FID 12.4"). If a magnitude matters, move the number into this node's "note" field, or file
  the numeric setting under context. Pattern is "which way did it move", separate from the
  metric name itself (eval_metric) and separate from the causal "why" (mechanism).
- "note" (optional, applies to ANY category): a short free-text memo holding a numeric
  magnitude or detail that was deliberately kept out of the trend/label — e.g. the exact delta
  a pattern moved ("+3.2 points", "FID 12.4 -> 9.1"), a compute figure, or a raw score. Leave
  "" when there is nothing to record. Numbers belong here (or in context), NEVER inside a
  pattern's text.
...
STEP 3 — Is this describing the direction/shape the measurements took across conditions (e.g.
"monotonic increase", "degradation from A to B", went up / down / flat)?
  YES -> "pattern" (category P). Put ONLY the trend in "text" (up/down/flat/mixed or a short
  shape phrase); strip any raw number out and put it in "note". If eval_metric and pattern are
  named in the same sentence ("FID dropped sharply, from 12.4 to 9.1, after 10k steps"), split
  into two nodes: "FID" -> E (eval_metric); "dropped sharply after 10k steps" -> P (pattern)
  with "note": "12.4 -> 9.1". Never keep them as one merged node, and never leave the number
...
- Cover the ENTIRE paper: abstract, body, EVERY figure and table (their content, not only
  their captions), and appendix.
...
## R5b — Figures and tables are first-class evidence (do NOT skim them)
This paper is attached as a PDF and you can SEE its figures and tables. Much of the real
evidence — especially interventions, contexts, and observables — lives in the visuals, not in
prose. Treat every figure and every table as an evidence unit and mine it directly:
- Walk each Figure and Table in turn. From what it SHOWS (grids, examples, curves, qualitative
  cases, a comparison), extract the AIO nodes it carries — typically:
  - intervention (I): the manipulation the visual demonstrates (e.g. "inputs presented as
    images rather than token grids", "ablate component X");
  - context (C): the task / dataset / setting the visual is drawn from;
  - eval_metric (E) + pattern (P): what the axis/column measures, and which DIRECTION it moves
    (up/down/flat/mixed) — put any number in "note", never in the pattern text (see Step 3);
  - mechanism (M): a "why" the visual makes visible (e.g. a failure case that reveals the cause).
- A figure that only illustrates qualitative behaviour still yields nodes: the shown behaviour
  is a pattern (P) on an (often implicit) metric, in a context (C), under an intervention (I).
- "location" MUST name the source figure/table id verbatim for any node grounded in a visual —
  e.g. "Figure 3", "Table 2" (this is what coverage is scored against). Use the paper's printed
  id exactly; if a node draws on both a figure and its caption, list both in "location".
- Do not invent results not shown. If a figure's metric is unlabeled, describe the axis in
  eval_metric as best you can and mark lower confidence rather than guessing a metric name.
...
"node_id", "source_span", "parent", "text", "location", "note", "classification_reasoning",
