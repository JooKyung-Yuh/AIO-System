...
- **(A) load-bearing AND testable here — CARD IT.**
  (e.g. "most ARC tasks are unambiguous" ← the multi-view / majority-voting results bear on it;
   "ARC colors are not real-world colors" ← the interpolation/patch ablations bear on it.)
- **(B) evaluation / scope / task-setup condition — DO NOT CARD** (leave for context). A description
  ...
  (e.g. "ARC tasks have very few demo pairs"; "single-view inference cannot produce multiple
   predictions"; "constraining a 2x2 patch to one raw pixel equals a 1x1 patch".)
