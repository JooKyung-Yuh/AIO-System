...
> 대상 v2 links: **133** (card-level, 5빌드)

## 새 ontology로 투영한 factor graph 구조

- **Observation → Claim(mechanism) direct**: 52  ← 진짜 belief_update (39%)
- **Claim → paper_thesis rolls_up**: 10 mechanism (+ 신규 aggregate_claim 3개)
...
- **unresolved (aggregate_claim 부재)**: 21 → 신규 claim 3개로 해소 제안
...
- **AGG_other_cumulative** — "Other cumulative/combined result"  (흡수 1 link, rolls_up→thesis)
    - ex: 'performing TTT independently for each test task yields subst'
...
- **aggregate_claim 3개만 신설**하면 21개 unresolved가 해소되고 집계 관측이 제자리를 찾음.
...
3. aggregate gap 대표패턴+신규 claim 제안: 3개 후보, 21 link 커버
4. 코드/프롬프트 변경: 없음 (docs-jk artifact only)